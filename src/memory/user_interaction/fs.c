...
uint16_t* disk_read(uint32_t LBA, uint32_t sector_count) {
    ...
    for(int i = 0; i < 4; i++) inb(0x1F7); //provide a 400ns delay so that the DRQ can set
    ...
    uint16_t* buffer = kmalloc(sector_count * 512);
...
void fs_init() {
    ...
    if(sb->magic_number == 0x19671972) {
        ...
        in the block array in the inode struct we have 12 blocks that can either be assigned to a file or directory
        */
    } else format_disk();
...
void format_disk() {
    ...
    //format disk bitmap
    disk_bitmap = (uint32_t*)kmalloc((sector_count + 7) / 8);
    //set the first 3 sectors as used
    for(uint32_t i = 0; i < (sector_count + 31) / 32; i++) disk_bitmap[i] = 0x0;
    disk_bitmap[0] = 0x0F;
    ...
    directories = (struct directory*)kmalloc(512); //allocate 1 sector for directory array
    ...
    //write disk bitmap
    disk_write(sb->disk_bitmap_LBA, 1, (uint16_t*)disk_bitmap);
